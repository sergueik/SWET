...
// origin: https://raw.githubusercontent.com/lcaron/opal/master/src/main/java/org/mihalis/opal/breadcrumb/BreadcrumbItem.java
...
public class BreadcrumbItem extends Item {
	...
	private static Color SELECTED_COLOR = new Color(display, 223, 220, 213);
	...
	private Point computeDefaultSize() {
		...
		return new Point(2 * MARGIN + sizeOfTextAndImages.x,
				2 * MARGIN + sizeOfTextAndImages.y);
	}
	...
	private void drawTrianglesAtPosition(final int x) {
		...
		if (this.parentBreadcrumb.hasBorder) {
			...
			this.gc.drawLine(x + getWidth(), this.toolbarHeight - 1,
					x + getWidth() + 3, this.toolbarHeight - 1);
		}
	}
