...
public class MultiChoiceEx<T> extends Composite {
	 ...
	 *                </ul>
	 */
	public MultiChoiceEx(final Composite parent, final int style) {
	...
	public MultiChoiceEx(final Composite parent, final int style,
		...
		this.filter = new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				if (shell == MultiChoiceEx.this.getShell()) {
	...
	private void createGlobalListener() {
		this.listener = new Listener() {
			@Override
			public void handleEvent(final Event event) {
				if (MultiChoiceEx.this.popup == event.widget) {
					handlePopupEvent(event);
					return;
				}

				if (MultiChoiceEx.this.arrow == event.widget) {
					handleButtonEvent(event);
					return;
				}

				if (MultiChoiceEx.this == event.widget) {
	...
	private void addListeners() {
		...
		if ((getStyle() & SWT.READ_ONLY) == 0) {
			final Listener validationListener = new Listener() {
				...
				@Override
				public void handleEvent(final Event event) {
					if (!MultiChoiceEx.this.popup.isDisposed()
							&& !MultiChoiceEx.this.popup.isVisible()) {
						validateEntry();
					}
				}
			};
	...
	protected void validateEntry() {
		...
		if (fieldsInError.size() == 1) {
			messageToDisplay = String.format("multi choice error %s",
					fieldsInError.get(0));
		} else {
			...
			messageToDisplay = String.format("multi choice message", sb.toString());
		}
		getDisplay().asyncExec(new Runnable() {
			@Override
			public void run() {
				final MessageBox mb = new MessageBox(getShell(),
						SWT.OK | SWT.ICON_ERROR);
				mb.setMessage(messageToDisplay);
				mb.open();
				MultiChoiceEx.this.text.forceFocus();
			}
		});
	...
	public Point computeSize(final int wHint, final int hHint,
		...
		final Point textSize = this.text.computeSize(SWT.DEFAULT, SWT.DEFAULT,
				changed);
		final Point arrowSize = this.arrow.computeSize(SWT.DEFAULT, SWT.DEFAULT,
				changed);
	...
	public void setSelectionListener(
			final MultiChoiceSelectionListener<T> selectionListener) {
		checkWidget();
	...
	private void createPopup() {
		...
		this.scrolledComposite = new ScrolledComposite(this.popup,
			...
			checkBoxButton.addSelectionListener(new SimpleSelectionAdapter() {
				...
				public void handle(final SelectionEvent e) {
					if (checkBoxButton.getSelection()) {
						MultiChoiceEx.this.selection.add(o);
					} else {
						MultiChoiceEx.this.selection.remove(o);
					}
					MultiChoiceEx.this.lastModified = o;
					setLabel();
				}
			});
	...
	private void changeVisibilityOfPopupWindow(final boolean show) {
		...
		final Rectangle parentRect = getDisplay().map(getParent(), null,
				getBounds());
		this.popup.pack();
	...
	private void checkNullElement() {
		if (this.elements == null) {
			throw new NullPointerException(
					"There is no element associated to this widget");
		}
	}
