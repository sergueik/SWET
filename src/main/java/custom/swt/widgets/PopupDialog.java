...
public class PopupDialog extends Window {
		...
		shell.addListener(SWT.Activate, new Listener() {
			public void handleEvent(Event event) {
				...
				if (event.widget == getShell() && getShell().getShells().length == 0) {
					...
					listenToParentDeactivate = !(osName.startsWith("Mac"));
				}
			}
		});
	...
	protected Control createTitleControl(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false)
				.span(showDialogMenu ? 1 : 2, 1).applyTo(titleLabel);
	...
	private void createDialogMenu(Composite parent) {
		...
		viewMenuButton
				.setDisabledImage(JFaceResources.getImage(POPUP_IMG_MENU_DISABLED));
		viewMenuButton
				.setToolTipText(JFaceResources.getString("PopupDialog.menuTooltip")); //$NON-NLS-1$
	...
	protected void showDialogMenu() {
		...
		listenToDeactivate = (osName.startsWith("windows")
